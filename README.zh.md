...
- 工具目录，列出每个可见工具名称、schema 大小、调用次数、成功次数、失败次数、平均耗时、P95 耗时、累计输入/输出字节数和配置的副作用分类；
- 点击工具后展开该工具保留的有界调用记录，列出开始时间、持续时间、输入/输出字节数、结果状态、副作用分类和安全的失败类别；
...
Web renderer 会先呈现产品标签，再提供调试细节。默认打开时显示运行时身份、紧凑状态徽章、一句话结论，以及一条只包含工具、schema、调用和失败的摘要行；文件与域名活动降为次级信息。声明与观察能力矩阵、归属提示都采用扁平区块，不再堆叠指标卡片。主要工具目录会默认显示在有界滚动区域中，列为 `Tool`、`State`、`Usage`、`Avg`、`P95`、`Input / Output`、`Effect`；State 只使用 `Idle`、`Active`、`Healthy`、`Failed`、`Discarded`，Usage 根据有界 report 显示调用、成功、失败或进行中的调用。点击工具后会展开该工具保留的逐次调用记录，显示开始时间、持续时间、输入/输出字节数、结果状态、副作用分类和安全失败类别。当前窗口全量调用轨迹仍然通过渐进展开进入。当前事件契约提供的是字节数，而不是 tokenizer 生成的真实 token 数，因此 UI 会标注 bytes，不会估算 token。这次视觉调整不会改变 Host report、Unicode fallback、snapshot schema、有界证据或原始 payload 保留策略。
